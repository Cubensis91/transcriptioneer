...
export function TaskCard({ task }: { task: MockTask }) {
  return (
    <div className="flex items-center gap-3 rounded-lg border border-border bg-surface px-4 py-3">
      <Checkbox defaultChecked={task.done} aria-label={`Marcar "${task.title}" como ${task.done ? "pendiente" : "hecha"}`} />
      <p className={cn("flex-1 text-sm", task.done ? "text-text-subtle line-through" : "text-text")}>
        {task.title}
      </p>
      {task.due && <span className="text-xs text-text-subtle">Vence {task.due}</span>}
